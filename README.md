...
NovelM 是连接轻书架服务的 Windows 桌面客户端，重点覆盖漫画创建、资料维护、分卷编辑和图片上传流程。应用需要连接轻书架 API；发布管理功能需要具有相应权限的轻书架账户。
...
- 使用账号密码登录，或导入 RefreshToken 与 `x-id` 恢复 Web 端会话。
...
1. 安装所需运行库并启动 NovelM。
2. 打开“账户”，使用邮箱与密码登录；也可以输入 RefreshToken 和 `x-id` 进行 Token 登录。
...
> [!IMPORTANT]
> Token 登录会替换本机保存的 `x-id` 和 RefreshToken。
